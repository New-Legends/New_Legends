...
{
  const motor_measure_t *chassis_motor_measure;
  pid_type_def rudder_motor_speed_pid;
  angle_PID_t rudder_motor_angle_pid;
  uint16_t offset_ecd;
...
typedef struct
{
  ...
  chassis_motive_motor_t chassis_motive_motor[4]; //chassis motor data.底盘动力电机数据
  chassis_rudder_motor_t chassis_rudder_motor[4];  //chassis rudder motor data.底盘舵向电机数据

  pid_type_def motive_motor_speed_pid[4];         //motor speed PID.底盘动力电机速度pid
...
/**
  * @brief          底盘舵向角度PID计算, 因为角度范围在(-pi,pi)，不能用PID.c的PID
  * @param[out]     pid:底盘舵向PID指针
  * @param[in]      get: 角度反馈
  * @param[in]      set: 角度设定
  * @param[in]      error_delta: 角速度
  * @retval         pid 输出
  */
extern fp32 chassis_rudder_PID_calc(angle_PID_t *pid, fp32 get, fp32 set, fp32 error_delta);
